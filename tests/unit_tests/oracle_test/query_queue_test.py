...
def test_FCFSQueryQueue(query_shape: tuple):
    bp = tm.TestBlueprint(process=DataSourceProcess(data_source=RandomUniformDataSource(query_shape=query_shape)),
                               evaluators=(tm.ConstrainEvaluator(func_path="oracles.process.add", q_index=slice(None,None,None)),
                                           tm.ConstrainEvaluator(func_path="oracles.process.pop", r_index=slice(None,None,None))))
